...
const path = {
  ...
  css: {
    ...
    dist: 'dist/css',
    watch: ['src/css/**/*.scss', 'src/blocks/**/*.scss'],
  },
  js: {
...
function css() {
  return gulp
    .src(path.css.src)
    .pipe(mode.dev(sourcemap.init()))
    .pipe(scss())
    .pipe(groupMedia())
    .pipe(autoprefixer())
    .pipe(mode.dev(sourcemap.write('.')))
...
const build = gulp.series(cleanBuild, gulp.parallel(html, css, js, img, webp, icons, fonts));
